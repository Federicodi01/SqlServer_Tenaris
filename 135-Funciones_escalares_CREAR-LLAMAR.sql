-- Especificamos el entorno de idioma para la sesión. El idioma de la sesión determina los formatos de fecha y hora y los mensajes del sistema.
set language us_english; 

-- Una empresa tiene almacenados los datos de sus empleados en una tabla denominada "empleados". Eliminamos la tabla, si existe y la creamos con la siguiente estructura:
...
if object_id('dbo.f_fechaCadena') is not null drop function dbo.f_fechaCadena;

go

-- Creamos una función a la cual le enviamos una fecha (de tipo varchar), en el cuerpo de la función se analiza si el dato enviado corresponde a una fecha, si es así, 
-- se almacena en una variable el mes (en español) y se le concatenan el día y el año y se retorna esa cadena; en caso que el valor enviado no corresponda a una fecha, la función retorna la cadena 'Fecha inválida':
