...
if object_id('pa_libros_ingreso') is not null drop procedure pa_libros_ingreso;
...
-- Empleamos un "if" para controlar el valor de la variable de retorno. No enviando al procedimiento valores:
declare @retorno int
exec @retorno=pa_libros_ingreso
if @retorno=1 select 'Registro ingresado'
 else select 'Registro no ingresado porque faltan datos';
